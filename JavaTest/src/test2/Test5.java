 ...
 *  4단계 : 위오 같이 반복 수행을 통해 검색하고자하는 값을 발견할 수 있다.
 */
public class Test5 {
	public static void main(String[] args) {
		...
		while(start <= end) {
			...
			int mid = (start + end) / 2; //위치값:4-5번째니까 35
			
			if(arr[mid] > value) {//35는 75보다 작으니까 왼쪽은 안 봄 그러니까 4번째에서 끝남
				end = mid - 1;
			}else if(arr[mid] < value) {//35보다 75가 크니까 오른쪽을 봄 5번째에서 시작
				start = mid + 1;
			}else {
				loc = mid;
				state = true;
